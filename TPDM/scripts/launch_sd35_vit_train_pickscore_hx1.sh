...
export RUN_NAME="sd35_vit_pnt_pickscore_hx1_$(date +'%Y%m%d_%H%M%S')"

OUTPUT_DIR="outputs/$(date +'%Y-%m-%d')/$RUN_NAME"

echo "Starting SD3.5 ViT TimePredictor training on PickScore (hx1)..."
...
python -m torch.distributed.run --nproc_per_node $NUM_GPUS --nnodes 1 --standalone \
    ...
    --model_config configs/models/sd35_pnt_vit_lora.yaml \

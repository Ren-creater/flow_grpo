...
export RUN_NAME="sd35_vit_image_pnt_pickscore_hx1_$(date +'%Y%m%d_%H%M%S')"

OUTPUT_DIR="outputs/$(date +'%Y-%m-%d')/$RUN_NAME"

echo "Starting SD3.5 ViT Image TimePredictor training on PickScore (hx1)..."
...
python -m torch.distributed.run --nproc_per_node $NUM_GPUS --nnodes 1 --standalone \
    ...
    --model_config configs/models/sd35_pnt_vit_image_lora.yaml \
    --reward_model_config configs/models/pickscore_reward.yaml \
    --train_dataset configs/datasets/pickscore_txt.yaml \
...
echo '--------------------------'
echo SD3.5 ViT Image TimePredictor training task done
echo '--------------------------'

# Optional: Run evaluation after training on PickScore test set
echo "Running final evaluation on PickScore test set..."
python -m torch.distributed.run --nproc_per_node $NUM_GPUS --nnodes 1 --standalone \
    eval_model.py \
    --model_config configs/models/sd35_pnt_vit_image_lora.yaml \
    --checkpoint_path $OUTPUT_DIR/final_checkpoint \
    --eval_dataset configs/datasets/pickscore_txt_eval.yaml \

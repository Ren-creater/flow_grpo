...
export RUN_NAME="sd35_vit_captions_$(date +'%Y%m%d_%H%M%S')"

OUTPUT_DIR="outputs/$(date +'%Y-%m-%d')/$RUN_NAME"

echo "Starting SD3.5 ViT TimePredictor training on aggregated captions..."
...
echo '--------------------------'
echo SD3.5 ViT Caption training task done
echo '--------------------------'

# Optional evaluation on a held-out prompt set
